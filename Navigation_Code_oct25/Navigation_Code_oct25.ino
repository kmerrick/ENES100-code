...
const int ROCK_LOCATION_X = 3400;
const int ROCK_LOCATION_Y = 550;
const int SEARCH_DISTANCE = 250;
...
void loop() {
  ...
  if (rf.receiveMarker(&marker, 169) {
    ...
    if (generalNavigation) {
      ...
      //TODO sweep out area;
      int startingAngle = tangent(marker.x, marker.y);
      turnTo(startingAngle);
    }
  }
}

int tangent(int currX, int currY) {
  int xComponent = ROCK_LOCATION_X - currX;
  int yComponent = ROCK_LOCATION_Y - currY;
  int tangentAngle = atan2(yComponent, xComponent);
...
void turnTo(int startingAngle) {
  int phi = fmod(marker.theta - startingAngle, 360);    //Rotates angle
  if (phi < 0) phi = phi + 360;                         //Eliminates negative angles
  if (phi < 180) turnLeft();
  else turnRight();
}
